				...
				getOutboundDetails({
					...
					if (response && response.data) {
						...
						this.productList = response.data.map(item => ({
							...
							code: item.partno || '',
							name: item.partname || '',
							receiveCount: item.invqty || 0,
							scannedCount: 0,
							status: item.invqty == item.scannedCount ? '完成' : '待扫'
						}));
						// 计算总进度
						this.calculateTotalProgress();
					} else {
